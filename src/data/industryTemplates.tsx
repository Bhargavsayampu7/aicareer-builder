...
// Non-Technical Industry Templates Data Store
...
export const getIndustryList = () => {
  return Object.entries(industries).map(([id, data]) => ({
    id,
    name: data.name,
    icon: data.icon,
    description: `Templates for ${data.name.toLowerCase()} professionals`,
    // Use the first template image as the industry image
    image: data.templates[0]?.image || "/placeholder.svg",
  }));
};
